...
if (version_compare(phpversion(), '5.3.1', '<')) {
    if (substr(phpversion(), 0, 5) != '5.3.1') {
        ...
        echo "Erebot_Module_TV requires PHP 5.3.1 or newer.
";
        exit -1;
    }
}
...
try {
    ...
    echo "Cannot process Erebot_Module_TV phar:
";
    echo $e->getMessage(), "
";
    exit -1;
}
function Erebot_Module_TV_autoload($class)
{
    $class = str_replace(array('_', '\\'), '/', $class);
    if (file_exists('phar://' . __FILE__ . '/Erebot_Module_TV-0.1.0/php/' . $class . '.php')) {
        include 'phar://' . __FILE__ . '/Erebot_Module_TV-0.1.0/php/' . $class . '.php';
    }
}
spl_autoload_register("Erebot_Module_TV_autoload");
$phar = new Phar(__FILE__);
$sig = $phar->getSignature();
define('Erebot_Module_TV_SIG', $sig['hash']);
define('Erebot_Module_TV_SIGTYPE', $sig['hash_type']);
